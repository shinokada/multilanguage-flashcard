<script lang="ts">
	...
	const randomIndexFn = randomNumberGenerator(1, 5034, 50);

	let randomIndex = $state(100);
...
let json
    $effect( () => {
    // Fetch and import the JSON data
    const response =  import(`../../../node_modules/emojibase-data/${myLang}/data.json`);
    // json = response.default;
    console.log(response)
  });
    // let motherLang = $state(json.default[randomIndex])
...
<Button onclick={random}>Random {randomIndex}</Button>

{#await import(`../../../node_modules/emojibase-data/${myLang}/data.json`) then json}
	<pre>{JSON.stringify(json.default[randomIndex], null, 2)}</pre>
{/await}
